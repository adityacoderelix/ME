...
const StatusPill = ({ status }) => {
  ...
  return (
    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(status)}`}>
      {status === "processing" ? "Sent for approval" : status.charAt(0).toUpperCase() + status.slice(1)}
    </span>
  )
}
...
const getUserPropertyListings = async (userEmail, page = 1, limit = 10) => {
  try {
    const response = await axios.get(`${API_BASE_URL}/properties/user-properties/${userEmail}`, {
      params: { page, limit },
    })
...
export function HostListingsTable({ userEmail }) {
  ...
  const handleConfirmDelete = useCallback(async () => {
    if (listingToDelete) {
      try {
        const response = await fetch(`${API_BASE_URL}/properties/user-property/${listingToDelete._id}`, {
          method: "DELETE",
        })
        ...
        setData((prevData) => prevData.filter((item) => item._id !== listingToDelete._id))
  ...
  const columns = React.useMemo(
    () => [
      ...
      },
      {
        ...
        header: "Created At",
        cell: ({ row }) => (
          <div className="text-center">{new Date(row.getValue("createdAt")).toLocaleDateString()}</div>
        ),
      },
      {
        id: "actions",
          ...
          return (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => router.push(`/host/dashboard/edit-listing/${listing._id}`)}>Edit listing</DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleDeleteClick(listing)}>Delete listing</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          )
        },
    ...
    <div className="w-full">
      ...
      <div className="rounded-md border">
        <Table>
          ...
          <TableBody>
            {loading ? (
              Array.from({ length: 10 }).map((_, index) => <SkeletonRow key={index} columns={columns} />)
