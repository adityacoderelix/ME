...
export const validateLocationForm = (formData) => {
  ...
  const isValid = isAddressValid && isRegistrationNumberProvided && validRegistrationNo;
...
export const validateBathroomSelector = (formData) => {
  const totalBathrooms = formData.bathroomTypes.private + 
...
export const validateOccupancySelector = (formData) => {
  return {
    ...
    errorMessage: "Please select at least one option for 'Who else might be there?'.",
  };
};
...
export const validateListingDetails = (formData) => {
  return {
    isValid: formData.title.trim().length >=3  && formData.description.trim().length >15,
    errorMessage: "",
  };
};
