...
export function SetRulesAndPublish({ updateFormData, formData }) {
  ...
  return (
    <div className="space-y-8 max-w-3xl mx-auto ">
      ...
      <div>
        <h3 className="text-base text-absoluteDark mb-2 font-bricolage">House Rules</h3>
        <div className="space-y-2">
          {defaultHouseRules.map((rule) => (
            <div key={rule.id} className="flex text-stone text-sm items-center space-x-2">
              ...
              />
              <Label className="text-sm font-normal" htmlFor={rule.id}>{rule.label}</Label>
            </div>
          ))}
          <div>
            <Label htmlFor="newRule" className="text-base text-absoluteDark font-bricolage mt-4 mb-2 ">Add custom rule</Label>
            <div className="flex flex-col  space-y-3 mt-1">
              ...
              />
              <Button className="w-32 bg-primaryGreen hover:bg-brightGreen h-10" onClick={addCustomRule} type="button">Add</Button>
            </div>
          </div>
          {customRules.length > 0 && (
            <div className="space-y-2 mt-6">
              <h4 className="text-base font-bricolage text-absoluteDark font-medium">Custom Rules:</h4>
              <ul className="list-disc list-inside">
                {customRules.map((rule, index) => (
                  <li key={index} className="flex items-center justify-between">
                    <span className='text-sm text-stone font-normal mb-4'>{rule}</span>
                    <button 
