...
export default function KYCVerification() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:5005/api/v1/pan-kyc/verify", {
      ...
      setError(err.response?.data?.error || 'An error occurred during KYC verification');
      setActiveStep(0);
    } finally {
      setLoading(false);
    }
      ...
      <CardContent>
        <div className="flex justify-between mb-8">
          {steps.map((label, index) => (
            <div key={label} className="flex flex-col items-center">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center ${index <= activeStep ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"}`}
              >
        ...
        {activeStep === 0 && (
          <div className="space-y-4">
            ...
            <Button onClick={handleSubmit} disabled={loading || !email || !image} className="w-full">
              {loading ? (
                <>
                  <svg
            ...
            {ocrResult && (
              <Card>
                ...
                <CardContent>
                  ...
                  <p>PAN Number: {ocrResult.result[0].details.pan_no.value}</p>
